...
public class DateUtil extends org.apache.commons.lang3.time.DateUtils {
    ...
    public static String getNextMonth(){
        ...
        int nextMonth = date.get(Calendar.MONTH) + 2;
        year += nextMonth / 12;
        nextMonth = nextMonth % 12;
        String strMonth = "";
        if (nextMonth < 10){
            strMonth = year + "-0" + nextMonth;
        }else {
            strMonth = year + "-" + nextMonth;
        }
        return strMonth;
    }
    ...
    public static String getCurrentMonth(){
        ...
        String strMonth = "";
        if (currentMonth < 10){
            strMonth = year + "-0" + currentMonth;
        }else {
            strMonth = year + "-" + currentMonth;
        }
        return strMonth;
    }

    public static String transfer(int second){
        String result = null;
        try {
    ...
    //旧日期加上月份之后的新日期
    public static Date addMonth(Date date, String month){
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(date);
     ...
     * @return: date
     */
    public static String getLastMonthDay(String date,String timeType,int monthNum){
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(stringToDate(date,timeType));
        calendar.add(Calendar.MONTH,-1 * monthNum);
        return dateToString(calendar.getTime(),timeType);
    ...
    /**
     * 日期字符串转换为日期 根据长度自动判断格式 4位年 7位年月 10位年月日 13位年月日时 16位年月日时分 19位年月日时分秒 需满足yyyy-MM-dd HH:mm:ss
     * @param dateStr 日期字符串
     * @return 日期
     */
    public static Date strToDate(String dateStr) {
        ...
        if (dateStr.length() == 4) {
            return cn.hutool.core.date.DateUtil.parse(dateStr, DatePattern.NORM_YEAR_PATTERN).toJdkDate();
        } else if (dateStr.length() == 7) {
            return cn.hutool.core.date.DateUtil.parse(dateStr, DatePattern.NORM_MONTH_PATTERN).toJdkDate();
        } else if (dateStr.length() == 10) {
            return cn.hutool.core.date.DateUtil.parse(dateStr, DatePattern.NORM_DATE_PATTERN).toJdkDate();
        } else if (dateStr.length() == 13) {
            return cn.hutool.core.date.DateUtil.parse(dateStr, "yyyy-MM-dd HH").toJdkDate();
        } else if (dateStr.length() == 16) {
            return cn.hutool.core.date.DateUtil.parse(dateStr, DatePattern.NORM_DATETIME_MINUTE_PATTERN).toJdkDate();
        } else if (dateStr.length() == 19) {
            return cn.hutool.core.date.DateUtil.parse(dateStr, DatePattern.NORM_DATETIME_PATTERN).toJdkDate();
        } else if (isValidUnixTime(dateStr)){
    ...
    // 格式化日期
    public static String formatDate(SimpleDateFormat dateFormat,Object date){
        if(date==null){
            return "";
        }else {
            ...
            Date d = strToDate(ConvertUtil.getString(date));
            return dateFormat.format(d);
        }
    }

    public static Boolean isValidUnixTime(String dateStr){
        SimpleDateFormat sdf = new SimpleDateFormat("EEE MMM dd HH:mm:ss zzz yyyy", Locale.ENGLISH);
        sdf.setTimeZone(TimeZone.getTimeZone("Asia/Shanghai"));
        try {
            sdf.parse(dateStr);
