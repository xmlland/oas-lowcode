...
public class FileUtil extends FileUtils {
    ...
    /**
     * Compress files to descFilePath if the file is a directory will compress all files in the directory to descFilePath
     *  if the file is a file will compress the file to descFilePath
     * @param filePaths
     * @param descFilePath
     */
    public static void zipFilesToZipFile(List<String> filePaths, String descFilePath) throws IOException {
        byte[] buf = new byte[4096];
        ...
        for (String filePath : filePaths) {
            ...
            if ( !file.exists() ){
                logger.warn("Failed to compressed file, source file " + filePath + " does not exist.");
                continue;
            }
            FileInputStream fin = null;
            ZipEntry entry = null;
            ...
            if (file.isDirectory()) {
                ...
                for (int i = 0; i < files.length; i++) {
                    if (files[i].isFile()) {
                        FileUtil.zipFilesToZipFile( filePath, files[i], zouts);
                    } else {
                        FileUtil.zipDirectoryToZipFile( filePath , files[i], zouts);
                    }
                }
            }else if (file.isFile()) {
                try {
    ...
    /**
     * @Description: 压缩文件夹 并输出到浏览器上
     */
    public static void zipDirectory(File directory, HttpServletResponse response,String fileName) {
        java.util.zip.ZipOutputStream zipOut = null;
        try {
            response.setContentType("APPLICATION/OCTET-STREAM");
            response.setHeader("Content-Disposition", "attachment; filename=\"" + new String(fileName.getBytes("UTF-8"), "ISO-8859-1") + "\"");
            zipOut = new java.util.zip.ZipOutputStream(response.getOutputStream());
            zipFiles(directory, directory, zipOut);
        }catch (Exception e){
            ...
            try {
                ...
                logger.warn("关闭ZIP输出流失败", e);
            }
        }
    }

    /**
     * @Description: 压缩文件夹下的所有文件
     */
    private static void zipFiles(File root, File file, java.util.zip.ZipOutputStream zipOut) {
        if (file.isDirectory()) {
            for (File innerFile : file.listFiles()) {
                zipFiles(root, innerFile, zipOut);
            }
        } else {
     ...
     * 根据url地址读取文件,并保存到磁盘上
     */
    public static void inputFileFromUrl(String fileHttpUrl, String savePath,String fileName){
        try {
            File director = new File(savePath);
            if(!director.exists()){
                director.mkdirs();
            }
            URL url = new URL(fileHttpUrl);
            ...
            logger.error("从URL下载文件失败", e);
        }
    }

    //压缩文件
    //被压缩路径:folderPath 压缩后输出到:outputPath 压缩后文件名: zipFileName
    public static void compressFolder(String folderPath, String outputPath, String zipFileName) throws IOException {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String filePath = "C:\\Users\\12038\\Desktop\\监管棉火车出库单.htm";

        String content = FileUtil.getCharset(filePath);

//        String content = FileUtil.getFileCharsetName(filePath);
