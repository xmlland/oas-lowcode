...
public class MyProcessDiagramGenerator {
    ...
    public InputStream generateDiagram(BpmnModel bpmnModel, String imageType, List<String> highLightedActivities, List<String> highLightedFlows,
        ...
        return generateProcessDiagram(bpmnModel, imageType, highLightedActivities, Collections.<String>emptyList(), highLightedFlows,
                activityFontName, labelFontName, annotationFontName, customClassLoader, scaleFactor).generateImage(imageType);
    }

    public InputStream generateDiagram(BpmnModel bpmnModel, String imageType, List<String> highLightedActivities, List<String> currentActivityIds, List<String> highLightedFlows,
                                       String activityFontName, String labelFontName, String annotationFontName, ClassLoader customClassLoader, double scaleFactor) {

        return generateProcessDiagram(bpmnModel, imageType, highLightedActivities, currentActivityIds, highLightedFlows,
                activityFontName, labelFontName, annotationFontName, customClassLoader, scaleFactor).generateImage(imageType);
    }
    ...
    protected MyProcessDiagramCanvas generateProcessDiagram(BpmnModel bpmnModel, String imageType,
                                                            List<String> highLightedActivities, List<String> currentActivityIds, List<String> highLightedFlows,
                                                            String activityFontName, String labelFontName, String annotationFontName, ClassLoader customClassLoader, double scaleFactor) {
        ...
        for (Process process : bpmnModel.getProcesses()) {
            for (FlowNode flowNode : process.findFlowElementsOfType(FlowNode.class)) {
                drawActivity(processDiagramCanvas, bpmnModel, flowNode, highLightedActivities, currentActivityIds, highLightedFlows, scaleFactor);
            }
        }
    ...
    protected void drawActivity(MyProcessDiagramCanvas processDiagramCanvas, BpmnModel bpmnModel,
                                FlowNode flowNode, List<String> highLightedActivities, List<String> currentActivityIds, List<String> highLightedFlows, double scaleFactor) {
        ...
        if (drawInstruction != null) {
            ...
            if (currentActivityIds.contains(flowNode.getId())) {
                drawCurrentHighLight(processDiagramCanvas, bpmnModel.getGraphicInfo(flowNode.getId()));
        ...
        for (SequenceFlow sequenceFlow : flowNode.getOutgoingFlows()) {
            ...
            String MyFlow = null;
            if (flowNode instanceof Activity) {
                MyFlow = ((Activity) flowNode).getDefaultFlow();
            } else if (flowNode instanceof Gateway) {
                MyFlow = ((Gateway) flowNode).getDefaultFlow();
            }

            boolean isMy = false;
            if (MyFlow != null && MyFlow.equalsIgnoreCase(sequenceFlow.getId())) {
                isMy = true;
            }
            ...
            if (graphicInfoList != null && graphicInfoList.size() > 0) {
                ...
                processDiagramCanvas.drawSequenceflow(xPoints, yPoints, drawConditionalIndicator, isMy, highLighted, scaleFactor);
            ...
            for (FlowElement nestedFlowElement : ((FlowElementsContainer) flowNode).getFlowElements()) {
                if (nestedFlowElement instanceof FlowNode) {
                    drawActivity(processDiagramCanvas, bpmnModel, (FlowNode) nestedFlowElement,
                            highLightedActivities, currentActivityIds, highLightedFlows, scaleFactor);
                }
            }
        }
    }
     ...
     * @return
     */
    protected static List<GraphicInfo> connectionPerfectionizer(MyProcessDiagramCanvas processDiagramCanvas, BpmnModel bpmnModel, BaseElement sourceElement, BaseElement targetElement, List<GraphicInfo> graphicInfoList) {
    ...
    protected void drawArtifact(MyProcessDiagramCanvas processDiagramCanvas, BpmnModel bpmnModel, Artifact artifact) {
    ...
    private static void drawCurrentHighLight(MyProcessDiagramCanvas processDiagramCanvas, GraphicInfo graphicInfo) {
        processDiagramCanvas.drawCurrentHighLight((int) graphicInfo.getX(), (int) graphicInfo.getY(), (int) graphicInfo.getWidth(), (int) graphicInfo.getHeight());
    }
