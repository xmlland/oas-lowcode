...
public class ERC20Executor {
	...
	public String transfer(Option option, String recipient, BigInteger amount) throws Exception {
		return this.account.call(option, new conflux.web3j.types.Address(this.contract), "transfer", new Address(recipient), new Uint256(amount));
	}
	
	public String approve(Option option, String spender, BigInteger amount) throws Exception {
		return this.account.call(option, new conflux.web3j.types.Address(this.contract), "approve", new Address(spender), new Uint256(amount));
	}
	
	public String transferFrom(Option option, String sender, String recipient, BigInteger amount) throws Exception {
		return this.account.call(option, new conflux.web3j.types.Address(this.contract), "transferFrom", new Address(sender), new Address(recipient), new Uint256(amount));
	}
