package com.jeestudio.tools.geo.kriging;
...
public class Kriging {
     ...
     * 方差参数
     * 平滑参数
     * | 参数 | 典型值范围 | 调整方向 | 效果变化 |<br/>
     * |---------|------------|------------------------|------------------------|<br/>
     * | sigma2 | 0~1 | 增大值 → 平滑性↑ | 插值结果更平滑 |<br/>
     */
     ...
     * | 参数 | 典型值范围 | 调整方向 | 效果变化 |<br/>
     * |---------|------------|------------------------|------------------------|<br/>
     * | alpha | 1~1000 | 减小值 → 模型复杂度↑ | 更能拟合数据细节 |
     */
    public double alpha = 100;

    // 训练数据
    private double[] targetValues; // 观测值数组
    private double[] xList;        // X坐标数组
    private double[] yList;        // Y坐标数组

    // 关键参数
    private double variogram_nugget = 0; // 块金值（测量误差）
    private double variogram_range = 0;  // 变程（空间相关性范围）
    private double variogram_sill = 0;   // 基台值（总方差）
    ...
    // 矩阵存储
    private double[] variogram_K; // 逆协方差矩阵
    private double[] variogram_M; // 权重矩阵
    ...
    /**
     * 训练方法,传入观测值数组，X坐标数组，Y坐标数组
     * @param targetValues 观测值数组
     * @param xList X坐标数组
     * @param yList Y坐标数组
     */
    public void train(double[] targetValues, double[] xList, double[] yList) {
        ...
        List<double[]> distances = new ArrayList<>(distance_capacity);
        int i, j, k, l;
        ...
        // 2. 对距离进行分箱处理（binning）
        int lags = Math.min(distance_capacity, 30);
        double tolerance = this.variogram_range / lags;
    ...
    /**
     * @Description: 创建网格，用于绘制等值线图
     * @param polygons 多边形数组
     * @param xWidth 网格x方向的宽度
     * @param yWidth 网格y方向的宽度
     **/
    public Grid grid(double[][][] polygons, double xWidth, double yWidth) {
        ...
        for (i = 0; i < len; i++) {
            if (targetValues[i] > maxValue) {
                maxValue = targetValues[i];
            }
            if (targetValues[i] < minValue) {
                minValue = targetValues[i];
            }
