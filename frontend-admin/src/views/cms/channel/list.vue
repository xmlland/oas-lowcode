<template>
  <TreeTableView :singleTable="singleTable" :modalComponent="modalComponent" modalTitle="栏目管理" :buttons="buttons" :rowButtons="rowButtons"
                 :queryArea="queryArea"
                 :class="useModernListSkin ? 'modern-tree-table-page cms-channel-tree-table-view' : ''"
                 formNo="prt_channel">
    <template v-if="useModernListSkin" #queryHeader>
      <div class="modern-list-query-heading">
        <h2 class="modern-list-query-title">栏目管理</h2>
        <p class="modern-list-query-desc">按层级维护 CMS 栏目、文章类型、状态和排序，支持添加下级栏目。</p>
      </div>
    </template>
  </TreeTableView>
</template>
...
let rowButtons = ref([{
  ...
  value: 'edit',
  text: '编辑',
  permission: 'cms:prt_channel:edit'
}, {
  value: 'delete',
  text: '删除',
...
let singleTable = ref({
  ...
  columns: [
    ...
    {title: '栏目名称', dataIndex: 'name', minWidth: 150, align: 'left'},
    {title: '文章类型', dataIndex: 'info_type', minWidth: 100, align: 'center', dict: 'prt_info_types'},
