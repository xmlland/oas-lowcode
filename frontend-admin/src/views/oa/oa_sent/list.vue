<template>
  <left-tree-right-table-view :rightTableView="rightTableView" modalTitle="我的请求" treeFormNo="oa_process_tree_setting"
                              :treeWidth="useModernListSkin ? 260 : 300"
                              :class="useModernListSkin ? 'modern-left-tree-right-page oa-sent-ltr-view' : ''"
                              dataUrl="dynamic/zform/getTaskList?path=sent">
    <template v-if="useModernListSkin" #queryHeader>
      <div class="modern-list-query-heading">
        <h2 class="modern-list-query-title">已发事项</h2>
        <p class="modern-list-query-desc">查看本人发起的流程事项，跟踪流程类型、当前节点和流转时间。</p>
      </div>
    </template>
...
let rightTableView=ref({
  url: {
    list: 'dynamic/zform/getTaskList?path=sent',
  },
