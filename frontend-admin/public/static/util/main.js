...
$(document).on("click", "div[component-type='grid'] .tag-div-button", function() {
	...
	var fieldDictId = $obj.attr("fieldDictId");
	...
	fieldType = fieldType ? fieldType.split(",") : [];
	fieldDictId = fieldDictId ? fieldDictId.split(",") : [];

	var selectSimple = $obj.attr("selectSimple");
	var selectValuefield = $obj.attr("selectValuefield");
	var selectDsf = $obj.attr("selectDsf");
	var selectOrder = $obj.attr("selectOrder");
	...
	var callbackFuc = $obj.attr("callback");
	var gridValidateFuc = $obj.attr("gridValidate");
	
	var initSearchValue = $obj.attr("initSearchValue")?$obj.attr("initSearchValue"):"";
	var filterSelectedData = $obj.attr("filterSelectedData") || false;

	var orderBy = $obj.attr("orderBy")?$obj.attr("orderBy"):"";
	layui.layer.open({
		type: 1,
		id: 'LAY-popup-iconSelect',
		url: 'admin/tag/grid',
		...
		success: function(layero, index) {
			...
			layui.view(_this.id).render(_this.url, typeof _this.initData === 'function' ? _this.initData.call(_this) : _this.initData || null).done(function() {
				...
				if(fieldDictId && fieldType && fieldType.length == fieldDictId.length){
					$.each(fieldKeys, function(idx, item) {
						columns.push({
							field: item,
							title: fieldLabels[idx],
							formatter: function(value, row, index) {
								if(fieldType[idx] == "select" || fieldType[idx] == "radio"){
									return myHelper.getValueFromDictionary(fieldDictId[idx],value);
								}else{
									return value;
								}
							}
				...
				var opt = {
					...
					queryParams: function(param) {
						...
						data.pageParam.orderBy = orderBy;
						data.tableName = tableName;
						data.searchKey = searchKey;
						//data.searchValue = $(layero).find("#searchValue").val();
						data.searchValue = (initSearchValue == "" ? $(layero).find("#searchValue").val() : initSearchValue);

						data.selectSimple = selectSimple;
						data.selectValuefield = selectValuefield;
						data.selectDsf = selectDsf;
		...
		yes: function(index, layero) {
			...
			if(filterSelectedData){
				...
				nameStr = nameStr.concat(nameArr);
			}
			if(ids.length == 0) {
				...
				//校验选中数据
				if(typeof window[gridValidateFuc] == "function"){
					if(!window[gridValidateFuc](resJson, $obj, index, ids, nameStr)){
						return;
					}
				}
...
$(document).on("click", "div[component-type='gridtabs'] .tag-div-button", function() {
	var _this = this;
	var $obj = $(_this).closest("div[component-type='gridtabs']");
	if($obj.attr("disabled")) {
		return false
	}
	var tableName = $obj.attr("tableName");
	...
	layui.layer.open({
		...
		yes: function(index, layero) {
			...
			var ids = $.map($(layero).find("#gridTag").bootstrapTable('getSelections'), function(row) {
				if(row[fieldKeys[0]]){
					returnRows.push({"id":row.id,"name":row[fieldKeys[0]]})
				}else if(fieldKeys[0].split(".").length > 0){
					returnRows.push({"id":row.id,"name":row[fieldKeys[0].split(".")[0]][fieldKeys[0].split(".")[1]]})
				}else{
					returnRows.push({"id":row.id,"name":row[fieldKeys[0]]})
				}
				return row.id
			...
			if(ids.length == 0) {
				...
				if(limit && ids.length > limit){
					LayerUtil.alert(layui.admin.lang() ? 'Only '+limit+' data can be selected' : '只能选择'+limit+'条数据');
					return false;
				}
				$obj.find("input[name='" + name + "']").val(ids.join(","));
				$obj.find("input[name='" + textName + "']").val(nameStr.join(","));
				$obj.find("div[name='content']").html("")
				$.each(returnRows, function(i,o){
					let html = `<div class="grid-tabs-div">
									<div class="grid-tabs-div-content">`+o.name+`</div>
									<div class="grid-tabs-div-x" ids="`+o.id+`" names="`+o.name+`">×</div>
								</div>`
					$(html).appendTo($obj.find("div[name='content']"))
				})
				ValidateUtil.closeTooltip($obj.find("input[name='" + textName + "']"));

				//校验选中数据
				if(typeof window[gridValidateFuc] == "function"){
					if(!window[gridValidateFuc](resJson, $obj)){
						return false;
					}
				}
...
//签发
$(document).on("click", "#report-table .qf-div .qf-btn-qf", function() {
	$(this).closest("div.adapt").find("textarea").val("签发");
});
//签发 清空
$(document).on("click", "#report-table .qf-div .qf-btn-clear", function() {
	$(this).closest("div.adapt").find("textarea").val("");
});
//会签常用语
$(document).on("click", "#report-table .hq-btn", function() {
	$(this).siblings(".hq-div").toggle();
});
//会签常用语 选择
$(document).on("click", "#report-table .hq-div>div", function() {
	var $textarea =$(this).closest("div.adapt").find("textarea");
	$textarea.val($textarea.val()+$(this).text());
	$(this).closest(".hq-div").hide();
});
$(document).on("click", "#report-table input[component-type='input-bh']", function() {
	var _this = this;
	if(!$(_this).val()){
		var formNo = $(_this).closest("#inputForm").find("#formNo").val();
		var sequence = $(_this).attr('data-sequence');
		if(sequence && $(_this).closest("#inputForm").find("input[name='"+sequence+"']").length>0){
			sequence = $(_this).closest("#inputForm").find("input[name='"+sequence+"']").val();
			if(!sequence){
				LayerUtil.warning("请先选择年份！");
				return false;
			}
		}else{
			sequence = "";
		...
		LayerUtil.ajax({
			url: layui.admin.basePath + '/oa/generateNumber/getNextNumber?formNo=' + formNo + '&sequence='+sequence,
			type: 'post',
			loading: true,
			success: function(result) {
				$(_this).val(result.data.nextNumber);
			}
		})
	}
});
...
//下载文单
$(document).on("click", "#download_paper", function() {
	var id = $(this).closest(".layui-tab-content").find("#id").val();
	var formNo = $(this).closest(".layui-tab-content").find("#formNo").val();
	console.log(formNo)
});
//下载正文
$(document).on("click", "#download_content", function() {
	var groupId = $(this).closest(".layui-tab-content").find("#contentGroupId").val();
	var src = layui.admin.basePath + "/oa/content/downloadContent?grounpId=" + groupId;
	window.open(src);
});

$(document).on("ifClicked", "input[type='radio']", function() {
	if($(this).iCheck('ifChecked')){
		$(this).iCheck('uncheck');
	}
});
