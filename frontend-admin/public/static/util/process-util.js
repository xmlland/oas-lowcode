...
var ProcUtil = {
    ...
    getUserList: function(paramStr, $table, saveUrl, isDatasourceUrl) {
    	var _this = this;
    	
    	if(saveUrl){
    		if(isDatasourceUrl){
				_this.saveUrl = layui.admin.datasourcePath + saveUrl;
			}else{
	...
	ruleArgsInit:function(dictionaryId){
		...
		//子表行可编辑控制
		var edittableflag = false;//不是所有的子表都需要行编辑
		$.each(formObj.find("table"),function(i,o){
			if($(o).attr("editTable")){
				edittableflag = true;
		...
		if(!zzb.viewBtn){
			styleStr += _this.$layero.selector+" .show-btn-group .view{display:none !important;} "
		}
		if(!zzb.editBtn){
