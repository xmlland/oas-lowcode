var LayerUtil = {
	...
	openDialogFlow: function(options) {
		...
		let yangSkin = ''
		if(isHidden == true){
			yangSkin = options.full ? ' layer-ext-yangskin' : 'wy-layer layer-ext-yangskin'
		}else{
			yangSkin = options.full ? '' : 'wy-layer'
		}
		var opt = {
			title: options.title,
			...
			url: options.url,
			area: [options.width || '1100px', options.height || '600px'],
			offset: $(".wy-layer").length > 0 ? [$(".wy-layer").last().css("top").replace("px", "") * 1 + 10 + "px", $(".wy-layer").last().css("left").replace("px", "") * 1 + 10 + "px"] : 'auto',
			id: options.id ? options.id : 'LAY-popup-' + (options.formLay ? options.formLay.replace(/@/g,'-'):""),
			shadeClose: false,
			skin: yangSkin,
			btn: btn,
			success: function(layero, index) {
				layui.view(this.id).render(options.url, typeof options.initData === 'function' ? options.initData.call(this) : options.initData || null).done(function() {
					...
					if(options.actionUrl) {
						LayerUtil.ajax({
							...
							success: function(res) {
									...
									if($.isPlainObject(value)) {
										$.each(value, function(vk, vv) {
											...
											switch(componentType) {
												...
												case "textarea-hq":	
													vv = vv.replace(/(\r\n|\n|\r)/gm,"<br>");
													$obj.find("[name='show" + k + "']").html(vv);
													$obj.find("[name='temp" + k + "']").val(data["temp"+k]);
													break;
												case "textarea-qf":	
													vv = vv.replace(/(\r\n|\n|\r)/gm,"<br>");
													$obj.val(vv);
										...
										switch(componentType) {
												...
												if(value) {
													LayerUtil.ajax({
														...
														success: function(result) {
															...
															if(data) {
																if(data.length > 0){
																	$obj.find("#groupIdForZipA").removeClass("hidden")
																}
																for(var i = 0; i < data.length; i += 1) {
																	...
																	if(componentAccept=="pic") {
																		if (data[i].url) {
																			html += "<img src='" + data[i].url + "?imageView2/4/h/100' />";
																		} else {
											...
											case "textarea-hq":	
												value = value.replace(/(\r\n|\n|\r)/gm,"<br>");
												$obj.find("[name='show" + k + "']").html(value);
												$obj.find("[name='temp" + k + "']").val(data["temp"+k]);
												break;
											case "textarea-qf":	
												value = value.replace(/(\r\n|\n|\r)/gm,"<br>");
								...
								let tabIndex = new Date().getTime();
								$.each($(layero).find("*[name]"),function(i,o){
								...
								$(layero).on("click", "#showProcImgDiv", function() {
									...
									if ($(layero).find("#procImgDiv").is(":visible")){
										$(layero).find("#procImgDiv").parent().css("margin-top","10px")
									}else{
										$(layero).find("#procImgDiv").parent().css("margin-top","0px")
									}
								});
								$(layero).on("click", "#showProcLogDiv", function() {
									$(layero).find("#procLogDiv").toggle();
									$(layero).find("#procImgDiv").hide()
									if ($(layero).find("#procLogDiv").is(":visible")){
										$(layero).find("#procLogDiv").parent().css("margin-top","10px")
									}else{
										$(layero).find("#procImgDiv").parent().css("margin-top","0px")
									}
								})
					...
					if(options.setDataWithoutAjax) {
						if(Object.prototype.toString.call( options.initData )==='[object Array]'){
							$(layero).find("table").bootstrapTable("load",options.initData)
						}else if(options.initData) {
							$.each(options.initData, function(k, v) {
								if(Object.prototype.toString.call( v )==='[object Array]'){
									$(layero).find("table").bootstrapTable("load", v)
		...
		var opt = {
			...
			success: function(layero, index) {
				layui.view(this.id).render(options.url, typeof options.initData === 'function' ? options.initData.call(this) : options.initData || null).done(function() {
					if(options.formLay) {
						_this.setInputFormLang(options.formLay, layero)
						$(layero).find(".adapt>label>span").remove()
					}
					$.each($(layero).find("#toolbarChild").find(".btn"),function (i,o) {
						$(o).remove()
					})
					$.each($(layero).find("table[edittable]"),function (i,o) {
						$(o).removeAttr("edittable")
					})
					typeof options.formInit === 'function' && options.formInit.call(this, typeof options.initData === 'function' ? options.initData.call(this) : options.initData || null);
					if(options.actionUrl && options.initData) {
						LayerUtil.ajax({
									...
									if($.isPlainObject(value)) {
										$.each(value, function(vk, vv) {
											...
											if($obj.attr("type") != "hidden") {
												...
												switch(componentType) {
													case "select":
														try{$obj.select2("destroy");}catch (e) {}
													...
													case "input":
														$obj.siblings(".tag-div-button").remove();
														$obj.replaceWith("<div class='form-control'>" + myHelper.escapeHTML(vv) + "</div>");
														break;
													case "textarea":
														$obj.siblings(".tag-div-button").remove();
														vv = vv.replace(/(\r\n|\n|\r)/gm,"<br>");
													...
													case "summernote":
														...
														break;
													case "textarea-hq":	
														$obj.closest("div.adapt").find(".hq-btn").remove();
														vv = vv.replace(/(\r\n|\n|\r)/gm,"<br>");
														$obj.replaceWith("<div class='form-control'>" + vv + "</div>");
														break;
													case "textarea-qf":	
														$obj.closest("div.adapt").find(".qf-div").remove();
														vv = vv.replace(/(\r\n|\n|\r)/gm,"<br>");
														$obj.replaceWith("<div class='form-control'>" + vv + "</div>");
														break;
													case "input-bh":	
										...
										if($obj.attr("type") != "hidden") {
											...
											switch(componentType) {
													...
													if(value) {
														LayerUtil.ajax({
															...
															success: function(result) {
																...
																if(data) {
																	if(data.length > 0){
																		html += '<a id="groupIdForZipA" ' +
																			'href="' + layui.admin.basePath + '/system/sysFile/fileDownloadZip?groupId='+value+'" ' +
																			'target="_blank">全部下载</a>';
																	}
																	for(var i = 0; i < data.length; i += 1) {
																		html += "<div>";
																		html += data[i].name;
												...
												case "input":
													$obj.siblings(".tag-div-button").remove();
													$obj.replaceWith("<div class='form-control'>" + myHelper.escapeHTML(value) + "</div>");
													break;
												case "textarea":
													$obj.siblings(".tag-div-button").remove();
													value = value.replace(/(\r\n|\n|\r)/gm,"<br>");
...
//			anim: 4
	...
	confirm: function(msg, succFuc, cancelFuc) {
		this.confirmWithBtn(msg, "", succFuc, cancelFuc)
	},
	confirmWithBtn: function (msg, btn, succFuc, cancelFuc) {
		if(btn == ""){
	...
	getBtnsStrFromMenuData: function(hashUrl, procBtns, removeBtns) {
		...
		if(tableBtns) {
			...
			YangTableBtnsStr += '<div class="show-btn-group">';
			$.each(tableBtns, function(i, obj) {
				if(!removeBtns){
					YangTableBtnsStr += '<a style="padding: 0px 10px;font-size: 12px;" class="btn ' + obj.sign + '" dealname="' + obj["pageName" + layui.admin.lang()] + '">' + obj["pageName" + layui.admin.lang()] + '</a>'
					...
					YangTableBtnsStr += '<a style="padding: 0px 10px;font-size: 13px;" class="btn ' + obj.sign + '" dealname="' + obj["pageName" + layui.admin.lang()] + '">' + obj["pageName" + layui.admin.lang()] + '</a>'
					tableBtnsStr += '<a class="btn ' + obj.sign + '" dealname="' + obj["pageName" + layui.admin.lang()] + '">' + obj["pageName" + layui.admin.lang()] + '</a>'
				}
			});
			tableBtnsStr += '</div></div>'
			YangTableBtnsStr += '</div>'
		}
		return {
			tableBtnsStr: tableBtnsStr,
			YangTableBtnsStr: YangTableBtnsStr,
