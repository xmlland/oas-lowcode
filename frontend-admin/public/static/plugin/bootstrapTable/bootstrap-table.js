(function (global, factory) {
	...
	*/
	...
	var DEFAULTS = {
	  ...
	  // bottom, top, both
	  paginationDetailHAlign: 'right',//wy
	  // right, left
	    ...
	    value: function initServer(silent, query, url) {
	      ...
	      var request = $.extend({}, Utils.calculateObjectValue(null, this.options.ajaxOptions), {
	        ...
	        success: function success(_res, textStatus, jqXHR) {
				...
				if (serverPublicKey && _res.data && typeof (_res.data) === 'string') {
					// eslint-disable-next-line no-undef
					_res = JSON.parse(Sm2Decrypt(_res.data, keyPairHex.privateKey))
				}
				if(_res.publicKey){
					sessionStorage.setItem("$serverPublicKey",_res.publicKey);
				}
