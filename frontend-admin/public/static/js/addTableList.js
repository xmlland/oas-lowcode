...
$(document).ready(function() {
	var _ctx = window['baseUrl'] || adminCtx.hostDev;
	//渲染select框
	$.ajax({
		type: 'post',
		...
		success:function(data){
			...
			if (serverPublicKey && data.data && typeof (data.data) === 'string') {
				// eslint-disable-next-line no-undef
				data = JSON.parse(Sm2Decrypt(data.data, keyPairHex.privateKey))
			}
			if(data.publicKey){
