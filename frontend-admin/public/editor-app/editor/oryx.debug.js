...
ORYX.CONFIG.TYPE_KISBPM_PRIORITY					= "kisbpm-priority";
ORYX.CONFIG.TYPE_KISBPM_CUSTOMCONDITION				= "kisbpm-customcondition";
ORYX.CONFIG.TYPE_KISBPM_CUSTOMCONDITIONOPERATOR		= "kisbpm-customconditionoperator";
...
ORYX.Core.StencilSet.StencilSet = Clazz.extend({
    ...
    construct: function(source, modelMetaData, editorId){
		...
		if(ORYX.CONFIG.BACKEND_SWITCH) {
			...
			// }
	// 		 var _this = this;
	// 		//alert(sessionStorage.getItem("$tokenBPM"));
	// 		var ajax = new XMLHttpRequest();
	// 		//步骤二:设置请求的url参数,参数一是请求的类型,参数二是请求的url,可以带参数,动态的传递参数starName到服务端
	// 		ajax.open('get',ACTIVITI.CONFIG.contextRoot + '/editor/stencilset?version=' + Date.now(),false);
    ...
    //                 };
    //                 ajax.responseText = JSON.stringify(data.data);
    //                 console.log(data)
    //                 this.responseText = JSON.stringify(data.data);
    //                 console.log(this.responseText);
	// 				_this._init.bind(this);
	// 			}else{
	// 				_this._cancelInit.bind(this)
	// 			}
	// 		}
	// 		ajax.send();
			//console.log(1111);
			new Ajax.Request(ACTIVITI.CONFIG.contextRoot + '/editor/stencilset?version=' + Date.now(), {
                method: 'get',
                asynchronous: false,
				//setRequestHeader:{"token": sessionStorage.getItem("$tokenBPM")}, 
    ...
    __handleStencilset: function(response){
        ...
        // console.log(typeof (JSON.parse(response.responseText).data));
        try {
...
function init() {
    ...
    // Else fetch the model from server and display editor
		...
		var modelUrl = ACTIVITI.CONFIG.contextBaseRoot + "/service/model/" + modelId + "/json";
...
ORYX.Editor = {
	...
	*/
	...
	setSelection: function(elements, subSelectionElement, force) {
		...
		//console.log("属性elements"+elements);
...
//		console.log("控制器"+elementController);
					...
					if (property != undefined) {
						...
						if(property.type() == ORYX.CONFIG.TYPE_CHOICE) {
							...
							property.refToView().each((function(ref) {
								...
								else if (property.type() === ORYX.CONFIG.TYPE_KISBPM_PRIORITY)
								{
									if (ref === "priority") {
										
										var svgElemAutomatic = this.node.ownerDocument.getElementById(this.id + '2');
										if(svgElemAutomatic) 
										{
											if (prop === '2')
											{
												svgElemAutomatic.setAttributeNS(null, 'display', 'inherit');
											}
											else
											{
												svgElemAutomatic.setAttributeNS(null, 'display', 'none');
											}
										} 
										var svgElemAutomatic = this.node.ownerDocument.getElementById(this.id + '3');
										if(svgElemAutomatic) 
										{
											if (prop === '3')
											{
										...
										var svgElemAutomatic = this.node.ownerDocument.getElementById(this.id + '4');
										if(svgElemAutomatic) 
										{
											if (prop === '4')
											{
										...
										var svgElemAutomatic = this.node.ownerDocument.getElementById(this.id + '5');
										if(svgElemAutomatic) 
										{
											if (prop === '5')
											{
								...
								else if (property.type() === ORYX.CONFIG.TYPE_KISBPM_CUSTOMCONDITION)
								{
									if (ref === "customcondition") {
										
										var svgElemAutomatic = this.node.ownerDocument.getElementById(this.id + 'Automatic');
										if(svgElemAutomatic) 
										{
											if (prop === 'Automatic')
											{
...
//										var svgElemExecutable = this.node.ownerDocument.getElementById(this.id + 'Executable');
//										if(svgElemExecutable) 
//										{
//											if (prop === 'Executable')
//											{
//												svgElemAutomatic.setAttributeNS(null, 'display', 'inherit');
...
//										} 
								...
								else if (property.type() === ORYX.CONFIG.TYPE_KISBPM_CUSTOMCONDITIONOPERATOR)
								{
									if (ref === "customconditionoperator") {
										
										var svgElemGt = this.node.ownerDocument.getElementById(this.id + 'GreaterThan');
										if(svgElemGt) 
										{
											if (prop === 'GreaterThan')
											{
												svgElemGt.setAttributeNS(null, 'display', 'inherit');
											}
											else
											{
												svgElemGt.setAttributeNS(null, 'display', 'none');
											}
										} 
										var svgElemLt = this.node.ownerDocument.getElementById(this.id + 'LessThan');
										if(svgElemLt) 
										{
											if (prop === 'LessThan')
											{
												svgElemLt.setAttributeNS(null, 'display', 'inherit');
											}
											else
											{
												svgElemLt.setAttributeNS(null, 'display', 'none');
											}
										} 
										var svgElemGtEq = this.node.ownerDocument.getElementById(this.id + 'GreaterThanOrEqual');
										if(svgElemGtEq) 
										{
											if (prop === 'GreaterThanOrEqual')
											{
												svgElemGtEq.setAttributeNS(null, 'display', 'inherit');
											}
											else
											{
												svgElemGtEq.setAttributeNS(null, 'display', 'none');
											}
										} 
										var svgElemLtEq = this.node.ownerDocument.getElementById(this.id + 'LessThanOrEqual');
										if(svgElemLtEq) 
										{
											if (prop === 'LessThanOrEqual')
											{
												svgElemLtEq.setAttributeNS(null, 'display', 'inherit');
											}
											else
											{
												svgElemLtEq.setAttributeNS(null, 'display', 'none');
											}
										} 
									}
									return;
